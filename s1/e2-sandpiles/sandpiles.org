...
pen =: 4
#+end_src
** intro text (talking head cam)

There have actually been a number of  papers written about
the mathematical properties of sandpiles,
but it was actually a physics paper that introduced them
to the world.

# show the paper
It appeared in the July 1987 issue of Physical Review Letters,
and was called _Self Organized Criticality: An Explanation of 1/f Noise_.
...
# show the white/pink/brown charts
Full disclaimer: I'm pretty much a beginner when it comes to
data analysis and statistics,
so all of these charts are from scholarpedia.org,
and if I'm screwing this part up, please let me
know in the comments, because I really struggled
to understand this part.

But as near as I can understand,
the idea of 1/f noise is that
it's halfway between purely random values
and a random walk.

So with white noise, every value is
completely random, and if you look at
the change in value between any two points,
it's just as likely to be zero
as the maximum distance.

So when you run white noise through
spectral analysis, you get a flat line.

Brown noise is when you have a random walk,
so at each step you flip a coin and either
move one step up or one step down. So if you
look at any two points in time, the distance
...
So these guys invented sandpiles
as a simple model of an interconnected system,
where each component affects its neighbors,
and they said okay, let's just fill the grid with random numbers
that are too big to be stable, let the simulation run,
and observe what happens when it settles down.

Their observation is that it self organizes
into a critical state, where adding a single
grain of sand (or just setting one of the cells to four)
is going to kick off a chain reaction
and if you look at distribution
of the sizes of those reactions,
you're going to see this 1/f behavior.
And then presumably this might give science a clue
about where to look for mechanisms
that cause this 1/f noise in nature.

Okay, well, I'm not sure that science is any closer
to understanding 1/f noise today, but I like this
idea of running experiments
on a computer simulation,
so let's see if we can reproduce their work.

** TODO the sandpaper experiment
#+begin_src j
cocurrent 'sandpaint'
