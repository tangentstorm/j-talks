...
So we'll start by overriding the title and the width of the palette.
So there's room to draw some labels in there.

And then =gpw_init= fires up our modified gridpad.
...
One thing I changed from the last video
is I added hooks in for mapping between the palette and rgb.

So the palette is just the raw numbers zero through four and then 8 16 32 and so on.

And the verb =to_rgb= maps numbers in the image to the matching interval.
For example, 11 falls between 8 and 16, so we color it like an 8.

This part in the middle says that if the number is out of range, just
map it to the last entry.

Then also, if you open a PNG file in here, =to_pal=
strips out the alpha channel
then maps these exact colors to those exact numbers.

And here, the part in the middle says treat any other color as zero.

Now in the interest of keeping this video short I'm not going
to explain how this works in detail.

For those of you who are actually interested in learning J,
I'm going to post a separate "code review" video for each one
of these applications, where I walk through the code line by line
